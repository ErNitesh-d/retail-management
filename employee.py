...
#root.title("Retail Manager(EMPLOYEE)")
...
def login(Event=None):
    ...
    if results:
        ...
        page2.time()
        biller.protocol("WM_DELETE_WINDOW", exit)
        biller.mainloop()
...
class login_page:
    def __init__(self, top=None):
        ...
        top.resizable(0, 0)
        top.title("Retail Manager(EMPLOYEE)")
...
class bill_window:
    def __init__(self, top=None):
        ...
        self.button6.configure(text="""EXIT/PRINT""")
        self.button6.configure(command=self.printt)
    ...
    def printt(self):
        sure = messagebox.askquestion("Print", "Do you want Print Bill ?")
        if sure == 'yes':
           url=tkinter.filedialog.asksaveasfile(mode='w',defaultextension='.txt')
           mob_no=self.num_message.get('1.0',END)
           bill_name=self.name_message.get('1.0',END)
           bill_no=self.bill_message.get('1.0',END)
           bill_date=self.bill_date_message.get('1.0',END)
           url.write('                         D-MART(FOODZILLA RETAIL LTD.)\n                               JUNAGADG ROAD,\n                               JETPUR-360370\n')
           url.write('BILL NUMBER: ')         
           url.write(bill_no)
           url.write('CUSTEMER NAME: ')                                                                                  
           url.write(bill_name)
           url.write('\nMOBILE NUMBER: ')
           url.write(mob_no)  
           url.write('DATE: ')
           url.write(bill_date)                                                                                                                                     
           url.write('\n--------------------------------------------------------------------------\n')
           url.write('PRODUCT NAME                           ')
           url.write('QTY                          ')
           url.write('PRICE                        \n')
           url.write('----------------------------------------------------------------------------\n')
           bill_detail=self.Scrolledtext1.get('1.0',END)
           divider = "\n\n\n" + ("─"*61)
           total = "\nTotal: Rs. {}".format(self.cart.total())                                      
           
           divider2 = "\n" + ("─"*61)
           bill_detail= bill_detail.replace(divider, "").replace(total,"").replace(divider2, "")
           total = "\nYOUR HAVE TO PAY TOTAL Rs.{}".format(self.cart.total())
           url.write(bill_detail)        
           url.write(total)
           url.close()
